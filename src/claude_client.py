...
# Effort levels per agent â Builder gets max, others get high
AGENT_EFFORT = {
    "builder": "max",
    "builder_step1": "max",
    "builder_step2": "max",
...
MAX_RETRIES = 3
RETRY_DELAYS = [30, 60, 120]  # seconds â exponential backoff


class ClaudeClient:
    """Tiered, cost-tracked Claude API client with effort parameter and retry."""
    ...
    async def call(
        ...
        """Call Claude with effort parameter, retry, and error capture."""
        ...
        # Thinking disabled â custom temperature per builder step for deterministic code gen
        # (Claude 4.6 constraint: thinking requires temperature=1, which conflicts with
        #  per-step temperatures like schema=0.2, API=0.2, core=0.3, auth=0.2, landing=0.4)
        api_kwargs = {
            "model": model,
        ...
        logger.info(
            "Calling %s (model=%s, tier=%s, effort=%s, max_tokens=%d, temp=%.1f)",
            agent_name, model, tier, effort, max_tokens, temperature,
        )
